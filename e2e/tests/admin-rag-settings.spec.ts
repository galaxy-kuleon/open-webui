...
test.describe('Admin RAG Settings — 10-control round-trip', () => {
	...
	// ---------------------------------------------------------------------------
	// UI round-trip: navigate to admin Documents page, toggle the
	// RAG_USER_COLLECTION_ENABLED Switch via the UI, save, intercept the
	// outgoing PUT, reload, assert the Switch reflects the persisted state.
	//
	// Selector: [data-testid="rag-user-collection-enabled-switch"] button[role="switch"]
	// Save trigger: button[type="submit"] with text "Save"
	// POST endpoint: POST /api/v1/retrieval/config/update
	// ---------------------------------------------------------------------------
	test('RAG_USER_COLLECTION_ENABLED switch: UI toggle → save → PUT body shape → reload → Switch state', async ({
		page
	}) => {
		// Step 1: capture original value so we can detect a true toggle
		const originalValue = originalRAG.RAG_USER_COLLECTION_ENABLED as boolean;
		const expectedAfterToggle = !originalValue;

		// Step 2: navigate to the admin Documents settings page
		await page.goto('/admin/settings/documents');

		// Wait for the RAGConfig to load — Switch becomes visible
		const switchWrapper = page.locator('[data-testid="rag-user-collection-enabled-switch"]');
		await switchWrapper.waitFor({ state: 'visible', timeout: 15_000 });

		// Step 3: arm route interception BEFORE clicking so we capture the request
		let capturedPutBody: Record<string, unknown> | null = null;
		await page.route('**/api/v1/retrieval/config/update', async (route) => {
			const request = route.request();
			capturedPutBody = request.postDataJSON() as Record<string, unknown>;
			// Continue the request — do not abort, the server must persist the value
			await route.continue();
		});

		// Step 4: click the Switch to toggle it
		const switchButton = switchWrapper.locator('button[role="switch"]');
		await switchButton.click();

		// Step 5: click Save (form submit)
		await page.getByRole('button', { name: 'Save' }).click();

		// Step 6: wait for the PUT interception to fire (route.continue fires after click)
		await page.waitForResponse(
			(response) =>
				response.url().includes('/api/v1/retrieval/config/update') &&
				response.request().method() === 'POST',
			{ timeout: 10_000 }
		);

		// Assert the intercepted PUT body contains the toggled value
		expect(capturedPutBody).not.toBeNull();
		expect(capturedPutBody!.RAG_USER_COLLECTION_ENABLED).toBe(expectedAfterToggle);

		// Step 7: reload the page and wait for the Switch to re-render
		await page.reload();
		await switchWrapper.waitFor({ state: 'visible', timeout: 15_000 });

		// Step 8: assert Switch visual state matches what was saved
		// bits-ui Switch.Root renders data-state="checked" or data-state="unchecked"
		const dataState = await switchButton.getAttribute('data-state');
		const actualStateAfterReload = dataState === 'checked';
		expect(actualStateAfterReload).toBe(expectedAfterToggle);
	});
