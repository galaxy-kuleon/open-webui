...
test.describe.serial("PDF to DOCX Translation Pipeline", () => {
  ...
  for (const tc of TRANSLATION_TESTS) {
    test(`translation [${tc.label}]: ${tc.sourceDescription} → ${tc.targetLang}`, async ({
      ...
      expect(
        hasPKHeader,
        `[${tc.label}] DOCX must start with PK zip header (50 4B 03 04), got: ${Array.from(header).map((b) => b.toString(16).padStart(2, "0")).join(" ")}`,
      ).toBe(true);
      ...
      expect(
        hasDocumentXml,
        `[${tc.label}] DOCX zip must contain word/document.xml`,
      ).toBe(true);
      ...
      expect(
        hasContentTypes,
        `[${tc.label}] DOCX zip must contain [Content_Types].xml`,
      ).toBe(true);
