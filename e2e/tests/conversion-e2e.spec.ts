...
test.describe.serial("PDF to DOCX Conversion Quality", () => {
  ...
  for (const pdf of TEST_PDFS) {
    test(`conversion quality [${pdf.label}]: ${pdf.description}`, async ({
      page,
    }) => {
      ...
      for (const ext of intermediateExts) {
        ...
        expect(
          count,
          `[${pdf.label}] No ${ext} intermediate files should appear in output`,
        ).toBe(0);
      }
      ...
      expect(
        hasPKHeader,
        `[${pdf.label}] DOCX must start with PK zip header (50 4B 03 04), got: ${Array.from(header).map((b) => b.toString(16).padStart(2, "0")).join(" ")}`,
      ).toBe(true);
      ...
      const { count, matched, missed } = countMarkerMatches(
        docxText,
        pdf.contentMarkers,
      );
      expect(
...
test.describe("Helper diagnostics", () => {
  test("countMarkerMatches: reports matched and missed markers accurately", () => {
    const text = "This contract between Party A and Party B covers Artificial Intelligence services.";
    const markers = ["Party A", "Party B", "Artificial Intelligence", "Taiwan Smart Technology", "AG-2025-TW-0042"];
