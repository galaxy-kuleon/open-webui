...
test.describe("Helper Contract Tests", () => {
  ...
  test("selectModel: can select OpenRouter model and button text changes", async ({
    page,
  }) => {
  ...
  test("createNewChat: navigates to home with chat input visible", async ({
    page,
  }) => {
    await login(page);
   ...
   */
  test.skip("sendMessage + waitForResponse: full chat round-trip contract", async ({
    page,
  }) => {
    await login(page);
    await createNewChat(page);
  ...
  test("CHAT_SELECTORS: all selectors match live DOM elements", async ({
    page,
  }) => {
    await login(page);
    await createNewChat(page);
    ...
    await expect(
      page.locator(CHAT_SELECTORS.modelSelectorButton),
    ).toBeVisible();
