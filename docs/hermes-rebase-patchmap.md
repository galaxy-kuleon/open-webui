...
| HERMES-HOOK-CONTINUATION-PIPE | `backend/open_webui/pipes/hermes_agent.py` | new SSE branch + `_emit_continuation` static method | 4 | Translate `event: hermes.continuation.suggested` SSE frame into `__event_emitter__` status event with `action="hermes_continuation"` + task_summary payload | load-bearing |
...
| HERMES-HOOK-MEMORY-TOOL-ROUTE | `gateway/platforms/api_server.py` | `_handle_memory_tool` method + `/v1/memory/tool` route registration | 3 | Admin/inspection endpoint dispatching to memory_manager.handle_tool_call with user_id/tenant_id kwarg scoping; loads configured provider on demand | load-bearing |
| HERMES-HOOK-CONTINUATION-HOOK | `run_agent.py` | `continuation_callback` kwarg on AIAgent.__init__ + self-assignment + invocation of agent._continuation_probe.maybe_emit_continuation after memory_manager.initialize_all | 4 | Capability-gated continuation probe: when a registered memory provider exposes a *_reasoning tool, ask about incomplete prior-session tasks; fires callback on non-sentinel response | load-bearing |
| HERMES-HOOK-CONTINUATION-SSE | `gateway/platforms/api_server.py` | `_on_continuation` closure in `_handle_chat_completions` + `__continuation__` queue tag in `_emit` → `event: hermes.continuation.suggested` SSE frame; callback threaded through `_run_agent` + `_create_agent` | 4 | Translate continuation callback invocation into a wire-level SSE event the openwebui pipe can parse | load-bearing |
